...
**Gin + Gorm + Mysql + Redis + Viper + Slog + RabbitMQ + RocketMQ + Promethus + Grafana + Crontab + Lua**
...
- **功能：** 通过 **SnowFlake** 生成**分布式ID**；
  - **认证模块：**
    - 发送验证码：
      ...
      - **QQ 邮箱 SMTP** 发送邮件验证码；
    - 登录：
      - 手机号码 + 验证码进行登录（未注册的手机号自动注册）；
      - 手机号码/邮箱 + 密码进行登录；
      - 退出登录；
    - 密码：
      - 设置初始密码（首次登录的手机号自动注册的用户）；
      - 修改密码；
  - **用户模块：** 个人主页、修改密码、修改个人资料、推荐关注；
  ...
  - **私信模块：** 支持一对一单聊私信；
  - **抽奖模块：** 支持高并发抽奖，单机压测 QPS 2850, avg time 79ms；
