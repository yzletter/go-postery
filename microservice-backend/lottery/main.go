...
func main() {
	...
	// 本地测试
	if *env == "local" {
		ServiceName = "test_lottery_service"
		GoPostery = "test_go_postery"
		EtcdEndPoint = "localhost:12379"
	} else {
		ServiceName = "lottery_service"
	...
	Config := conf.LoadGlobalConfig(ctx, EtcdClient, ServiceName+"_", GoPostery+"_") // Get Config From Remote Config Center
