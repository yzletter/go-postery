...
// Create 创建 Post
func (dao *gormPostDAO) Create(ctx context.Context, post *model2.Post, events []*model2.Event) error {
	// 0. 兜底
...
func (dao *gormPostDAO) Delete(ctx context.Context, id int64) error {
	// 1. 操作数据库
	now := time.Now()
	result := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("id = ? AND deleted_at IS NULL", id).Update("deleted_at", &now)
	if result.Error != nil {
		// 系统层面错误
...
func (dao *gormPostDAO) UpdateCount(ctx context.Context, id int64, field model2.PostCntField, delta int) error {
	// 1. 获取更新列名
	...
	// 2. 操作数据库
	result := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("id = ? AND deleted_at IS NULL", id).UpdateColumn(col, gorm.Expr(col+" + ?", delta))
	if result.Error != nil {
		// 系统层面错误
	...
	if result.RowsAffected == 0 {
		// 业务层面错误
		var cnt int64
		result2 := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("id = ? AND deleted_at IS NULL", id).Count(&cnt)
...
func (dao *gormPostDAO) Update(ctx context.Context, id int64, updates map[string]any) error {
	// 1. 操作数据库
	result := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("id = ? AND deleted_at IS NULL", id).Updates(updates)
	if result.Error != nil {
		// 系统层面错误
...
func (dao *gormPostDAO) GetByID(ctx context.Context, id int64) (*model2.Post, error) {
	// 1. 构造结构体对象
	post := &model2.Post{}
...
func (dao *gormPostDAO) GetByUid(ctx context.Context, id int64, pageNo, pageSize int) (int64, []*model2.Post, error) {
	// 0. 兜底
	...
	// 1. 操作数据库
	base := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("user_id = ? AND deleted_at IS NULL", id)
...
// GetByPage 按页查找 Post
func (dao *gormPostDAO) GetByPage(ctx context.Context, pageNo, pageSize int) (int64, []*model2.Post, error) {
	// 0. 兜底
	if pageNo < 1 || pageSize <= 0 || pageSize > 100 {
		return 0, nil, ErrParamsInvalid
	}
	...
	// 1. 操作数据库
	base := dao.db.WithContext(ctx).Model(&model2.Post{}).Where("deleted_at IS NULL")
...
func (dao *gormPostDAO) GetByPageAndTag(ctx context.Context, tid int64, pageNo, pageSize int) (int64, []*model2.Post, error) {
	// 0. 兜底
	if pageNo < 1 || pageSize <= 0 || pageSize > 100 {
		return 0, nil, ErrParamsInvalid
	}
