...
func TestCreatePost(t *testing.T) {
	...
	pid, err := database.CreatePost(9, "洛神赋", content)
	if err != nil {
		fmt.Println(err)
	} else {
...
func TestUpdatePost(t *testing.T) {
	err := database.UpdatePost(5, "测试", "测试测试测试测试测试测试测试")
	if err != nil {
		fmt.Println("修改失败，")
	} else {
		fmt.Println("修改成功")
	...
	err = database.UpdatePost(1, "测试", "测试测试测试测试测试测试测试")
	if err != nil {
...
func TestGetPostByPage(t *testing.T) {
	pageNo, pageSize := 1, 2
	fmt.Println("--------------", pageNo, "--------------")
	total, news := database.GetPostByPage(pageNo, pageSize)
	if total == 0 {
