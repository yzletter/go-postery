...
local member = ARGV[4]                     -- 当前请求唯一标识
local startTime = now - duration           -- 起始时间

redis.call('ZREMRANGEBYSCORE', key, '-inf', startTime)      -- 移除 startTime 之前所有值
local cnt = redis.call('ZCOUNT', key, '-inf', '+inf')       -- 统计当前请求方有效请求次数

if cnt >= threshold then                   -- 当前请求方有效请求次数 >= 阈值, 执行限流
    return "true"
else                                       -- 当前请求方有效请求次数 < 阈值, 不执行限流
    redis.call('ZADD', key, now, member)   -- score 用时间, member 用唯一请求标识
