...
import (
	...
	infraSlog "github.com/yzletter/go-postery/micro-backend/outbox/infra/slog"
)
...
func main() {
	...
	// Infra 层
	_ = infra.Init(Config.MySQL)                                            // 初始化 MySQL
	KafkaProducer := infraKafka.InitProducer([]string{conf2.KafkaEndpoint}) // 初始化 Kafka 生产方

	// 开启协程
	go infra.ScanOutbox(ctx, KafkaProducer) // 开启扫表发消息协程
