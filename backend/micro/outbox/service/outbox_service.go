...
import (
	...
	"github.com/yzletter/go-postery/backend/micro/outbox/model"
	"gorm.io/gorm"
)
		...
		case <-ticker.C:
			...
			for _, event := range events {
				...
				if err != nil {
					...
					result = db.Model(&model.Event{}).Where("id = ?", event.ID).Updates(updates)
					if result.Error != nil {
				...
				// 发送成功回填表
				result = db.Model(&model.Event{}).Where("id = ?", event.ID).Update("status", 1)
				if result.Error != nil {
					slog.Error("Update Event Failed", "id", event.ID, "error", result.Error)
				}
			}
