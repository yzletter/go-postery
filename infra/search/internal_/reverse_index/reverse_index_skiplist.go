...
type SkipListReverseIndex struct {
	table *util.ConcurrentMap // 手写并发安全 Map, 每个 Key 对应一条跳表
...
func NewSkipListReverseIndex(DocumentNumEstimate int) *SkipListReverseIndex {
	...
	mp := util.NewConcurrentMap(runtime.NumCPU()*500, DocumentNumEstimate)
...
func (index *SkipListReverseIndex) Add(document *model.Document) {
	...
	//	sklValue := SkipListNodeValue{document.DocID, document.BitsFeature}
	//	if value, exists := index.table.Get(key); exists {
	//		list := value.(*skiplist.SkipList)
	//		list.Set(document.IndexID, sklValue) //IntId作为SkipList的key，而value里则包含了业务侧的文档id和BitsFeature
	//	} else {
