...
func Init(brokers []string, topic string, groupID string) *Kafka {
	return &Kafka{
		Reader: kafka.NewReader(kafka.ReaderConfig{
			...
			CommitInterval: conf.KafkaCommitInterval, // 每隔多长时间自动commit一次offset。即一边读一边向kafka上报读到了哪个位置
		...
		Writer: &kafka.Writer{
			...
			RequiredAcks:           kafka.RequireOne,         // RequireNone不需要等待ack返回，效率最高，安全性最低；RequireOne只需要确保Leader写入成功就可以发送下一条消息；RequiredAcks需要确保Leader和所有Follower都写入成功才可以发送下一条消息。
			AllowAutoTopicCreation: true,                     // Topic不存在时自动创建。生产环境中一般设为false，由运维管理员创建Topic并配置partition数目
			WriteTimeout:           conf.KafkaCommitInterval, // 设定写超时
		},
	}
}
