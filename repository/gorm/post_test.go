...
func TestCreatePost(t *testing.T) {
	...
	pid, err := database2.CreatePost(9, "洛神赋", content)
	if err != nil {
		fmt.Println(err)
	} else {
		fmt.Println("帖子发布成功  id 为", pid)
	}
}

func TestDeletePost(t *testing.T) {
	err := database2.DeletePost(1)
	if err != nil {
...
func TestUpdatePost(t *testing.T) {
	err := database2.UpdatePost(5, "测试", "测试测试测试测试测试测试测试")
	if err != nil {
		fmt.Println("修改失败，")
	} else {
		fmt.Println("修改成功")
	...
	err = database2.UpdatePost(1, "测试", "测试测试测试测试测试测试测试")
	if err != nil {
...
func TestGetPostByID(t *testing.T) {
	post := database2.GetPostByID(5)
	fmt.Printf("%+v\n", post)
}

func TestGetPostByPage(t *testing.T) {
	pageNo, pageSize := 1, 2
	fmt.Println("--------------", pageNo, "--------------")
	total, news := database2.GetPostByPage(pageNo, pageSize)
	if total == 0 {
...
// go test -v ./database/gorm -run=^TestCreatePost$ -count=1
// go test -v ./database/gorm -run=^TestDeletePost$ -count=1
// go test -v ./database/gorm -run=^TestUpdatePost$ -count=1
// go test -v ./database/gorm -run=^TestGetPostByID$ -count=1
// go test -v ./database/gorm -run=^TestGetPostByPage$ -count=1
